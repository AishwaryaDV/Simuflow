...
const BASE_HR: Partial<Record<NodeType, number>> = {
  ...
  [NodeType.CDN]:               0.010,   // CloudFront base
...
const PER_MILLION_REQ: Partial<Record<NodeType, number>> = {
  ...
  [NodeType.Database]:       0.200,
  [NodeType.Queue]:          0.400,   // SQS
  [NodeType.CDN]:            0.0085,  // CloudFront
  [NodeType.ApiGateway]:     3.500,   // API Gateway
  [NodeType.Serverless]:     0.200,   // Lambda
  [NodeType.PubSub]:         0.500,   // SNS
  [NodeType.ObjectStore]:    0.400,   // S3 requests
  [NodeType.LLMGateway]:    30.000,   // ~$0.03/1k tokens → rough req equiv
}
...
class CostStore {
  ...
  constructor() {
    ...
    reaction(
      ...
      (status) => { if (status === SimulationStatus.Idle) this._reset() },
    )
  }
  ...
  get spentPct(): number {
    return Math.min(100, (this.spentUsd / Math.max(this.budgetUsd, 0.001)) * 100)
  }
  ...
  _accumulate() {
    ...
    if (tick <= this._lastTick || simulationStore.status === SimulationStatus.Idle) return
    this._lastTick = tick
